...
w = 5
time_to_maturity = [30 for j in range(n_corporates)]
discount_rate = 0.2
beta = 0.8
delta = [np.random.triangular(0.1,0.2,0.3) for j in range(n_investors)] #0.3,0.5,0.7 #0.4,0.6,0.8 #0.1,0.2,0.3
mi = [np.random.triangular(0.01,0.011,0.012) for j in range(n_investors)]
sigma = [np.random.triangular(0.2,0.3,0.4) for j in range(n_investors)]
gamma = 0.7
...
Model = Equity_Debt_2.Equity_Debt(n_investors,w,n_corporates,n_equity,initial_value,time_to_maturity,discount_rate,face_value,beta,delta,mi,sigma,gamma,eta,noise,rho,iterations)
...
for t in range(iterations-1):
    d_1.append(P_shocked[0][t]-R_shocked[0][t])
    d_2.append(P_shocked[1][t]-R_shocked[1][t])
...
d_2 = []
for t in range(iterations-1):
    d_1.append(P_unshocked[0][t]-R_unshocked[0][t])
    d_2.append(P_unshocked[1][t]-R_unshocked[1][t])
