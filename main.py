...
class Equity_Debt:
    ...
    def __init__(self,n_investors,w,n_corporates,n_equity,initial_value,time_to_maturity,discount_rate,face_value,beta,delta,mi,sigma,gamma,eta,noise,rho,iterations,sim):
    ...
    def adjust_expectations(self,i):
        if i == 1:
            self.delta = [np.random.triangular(0.6,0.7,0.8) for j in range(self.n_investors)]
        else:
            self.delta = [np.random.triangular(0.2,0.3,0.4) for j in range(self.n_investors)]

    def adjust_debt(self,i):
        if i == 1:
            self.face_value = [0.2*self.initial_value[j] for j in range(self.n_corporates)]
        else:
            self.face_value = [0.5*self.initial_value[j] for j in range(self.n_corporates)]

    def adjust_parspace(self,i,j):
        delta = np.linspace(0.11,0.89,num=self.sim)
        b = np.linspace(0.1,0.9,num=self.sim)
        self.delta = [np.random.triangular(delta[i]-0.1,delta[i],delta[i]+0.1) for k in range(self.n_investors)]
        self.face_value = [b[j]*self.initial_value[m] for m in range(self.n_corporates)]
