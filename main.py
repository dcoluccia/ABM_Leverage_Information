...
class Equity_Debt:
    ...
    def Expected_assets(self,i,j,t):
        ...
        Expected_assets = []
        for j in range(self.n_corporates):
            GBM = self.initial_value[j]*np.exp((mean_vector[j]*(dt*(t-1))))
    ...
    def update_endowments(self,t):
        ...
        for j in range(self.n_corporates):
            ...
            #new_x.append(np.log(np.divide(self.true_returns[j],self.old_true_returns[j])))
