...
mi = [np.random.triangular(0.015,0.020,0.025) for j in range(n_investors)] #0.015,0.020,0.025
sigma = [np.random.triangular(0.1,0.15,0.20) for j in range(n_investors)] #0.01,0.015,0.020 #0.05, 0.075, 0.1
gamma = 0.6
eta = [0 for j in range(n_investors)] #[np.random.triangular(0.3,0.5,0.7) for j in range(n_investors)]
noise = 0
rho = 0.1
iterations = 60
face_value = [0.8 for j in range(n_corporates)]#[0 for j in range(M)] #
sim = 1
####################
...
for index in range(3):
    if index == 0: #mid chartist
        ...
        for t in time_periods:
            Model.adjust(t)
            Model.adjust_expectations(index)
            Model.compute_weight_matrix(t)
            Model.market(t)
            Model.update_endowments(t)
            r.append(np.average(Model.true_returns));p.append(np.average(Model.P[t+1]));a.append(np.average(Model.assets[t]))
...
##############
plt.figure(1)
plt.plot(Spread[1],'b',label='theta = 0.7');plt.plot(Spread[0],'r',label='theta = 0.5');plt.plot(Spread[2],'g',label='theta = 0.3')
plt.legend();plt.title('Price spreads')
plt.figure(2)
plt.plot(Leverage[1],'b',label='theta = 0.7');plt.plot(Leverage[0],'r',label='theta = 0.5');plt.plot(Leverage[2],'g',label='theta = 0.3')
